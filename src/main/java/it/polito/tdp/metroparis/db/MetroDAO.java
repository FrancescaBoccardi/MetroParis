...
public class MetroDAO {
	...
	public boolean fermateCollegate(Fermata f1, Fermata f2) {
		...
		try {
			...
			rs.first(); //si posiziona sulla prima riga del ResultSet
			
			int conteggio = rs.getInt("cnt"); //estraggo il conteggio
	...
	public List<Connessione> getAllConnessioni(List<Fermata> fermate){
		...
		String sql = "SELECT id_connessione, id_linea, id_stazP, id_stazA "
					+"FROM connessione "
					+"WHERE id_stazP>id_stazA"; // per evitare di trovare due volte la stessa connessione
		...
		try {
			...
			while(rs.next()) {
				...
				Connessione c = new Connessione(rs.getInt("id_connessione"),
						null, // ignoro la linea perché per ora non mi serve
						fermata_partenza,
						fermata_arrivo);
				result.add(c);
			}
